...
class PriorityQueue extends Queue {
    // we have access to every single method from Queue

    // overwrite Queue's version of enqueue()
    enqueue(item) {
        for(let i = 0; i < this.items.length; i++)
        {
            // compare .priority from each dictionary
            if(item.priority < this.items[i].priority) 
            {
                // splice(starting index, replace nothing, what to insert)
                this.items.splice(i,0,item);
                return this;
            }
        }
        // if we reach this point in the method, it means we never found a lower priority
        this.items.push(item);
        return this;
    }
    
    // overwrite Queue's version of print()
    print(){
        for (let index = 0; index < this.items.length; index++) {
            console.log(this.items[index].condition);
        }
        return this;
    }
}
...
testQ.enqueue({"condition":"broken leg", "priority":3})
testQ.enqueue({"condition":"broken hip", "priority":2})
testQ.enqueue({"condition":"heart attack", "priority":1})
testQ.enqueue({"condition":"amputated arm", "priority":2})
testQ.print();
// heart attack, broken hip, amputated arm, broken leg
// broken hip < amputated arm because they're both priority 2, but hip was here first
